    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-16 pt-4">
      ...
      <div className="space-y-4 border-b border-zinc-200/60 dark:border-zinc-800/60 pb-8">
        <div className="flex items-center gap-3 text-xs font-mono text-zinc-500">
          <span className="px-2.5 py-0.5 rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 font-medium border border-zinc-200/60 dark:border-zinc-700/60">
            {project.category}
          </span>
          <span>•</span>
        ...
        <div className="flex flex-wrap items-center gap-3 pt-2">
          <a
            ...
            className="inline-flex items-center gap-2 px-4 py-2 text-xs sm:text-sm font-medium rounded-xl backdrop-blur-md bg-zinc-900/90 dark:bg-zinc-100/90 text-white dark:text-zinc-900 hover:bg-zinc-800 dark:hover:bg-white transition-all shadow-sm"
          >
          ...
          {project.liveUrl && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 text-xs sm:text-sm font-medium rounded-xl backdrop-blur-md bg-white/60 dark:bg-zinc-900/60 text-zinc-900 dark:text-zinc-100 border border-zinc-200/80 dark:border-white/10 hover:bg-zinc-100/80 dark:hover:bg-zinc-800 transition-colors shadow-xs"
            >
        ...
        {project.stats && (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-4">
            {project.stats.map((stat, idx) => (
              <div
                key={idx}
                className="p-3.5 rounded-xl backdrop-blur-sm bg-white/60 dark:bg-zinc-900/40 border border-zinc-200/60 dark:border-white/10 shadow-xs"
              >
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="p-6 rounded-2xl backdrop-blur-md bg-white/60 dark:bg-zinc-900/40 border border-zinc-200/60 dark:border-white/10 space-y-3 shadow-sm">
          <h2 className="text-sm font-mono font-semibold uppercase tracking-wider text-zinc-700 dark:text-zinc-300 flex items-center gap-1.5">
            <AlertTriangle className="w-4 h-4 text-zinc-500" /> The Problem
          </h2>
        ...
        <div className="p-6 rounded-2xl backdrop-blur-md bg-white/60 dark:bg-zinc-900/40 border border-zinc-200/60 dark:border-white/10 space-y-3 shadow-sm">
          <h2 className="text-sm font-mono font-semibold uppercase tracking-wider text-zinc-700 dark:text-zinc-300 flex items-center gap-1.5">
            <CheckCircle2 className="w-4 h-4 text-zinc-500" /> The Solution
          </h2>
          <p className="text-xs sm:text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">
            {project.solution}
          </p>
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {project.keyFeatures.map((feature, idx) => (
            <div
              key={idx}
              className="p-4 rounded-xl border border-zinc-200/60 dark:border-white/10 backdrop-blur-md bg-white/60 dark:bg-zinc-900/40 text-xs sm:text-sm text-zinc-700 dark:text-zinc-300 flex items-start gap-2.5 shadow-xs"
            >
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-4">
          {project.challenges.map((item, idx) => (
            <div
              key={idx}
              className="p-5 rounded-2xl backdrop-blur-md bg-white/60 dark:bg-zinc-900/40 border border-zinc-200/60 dark:border-white/10 space-y-2.5 shadow-sm"
            >
              <h3 className="text-xs sm:text-sm font-semibold text-zinc-900 dark:text-zinc-100">
                <span className="font-mono text-zinc-500 mr-2">Challenge #{idx + 1}:</span>
                {item.challenge}
              </h3>
              <p className="text-xs sm:text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed pl-4 border-l-2 border-zinc-300 dark:border-zinc-700">
      ...
      {relatedArticles.length > 0 && (
        <section className="space-y-4 pt-6 border-t border-zinc-200/60 dark:border-zinc-800/60">
