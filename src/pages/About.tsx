...
export const About: React.FC = () => {
  return (
    <div className="max-w-3xl mx-auto space-y-12 pb-16 pt-6">
      {/* Header */}
      <div className="space-y-3 border-b border-slate-200/80 dark:border-zinc-800/80 pb-8">
        ...
        <div className="space-y-3 text-slate-700 dark:text-zinc-300 text-sm sm:text-base leading-relaxed">
          ...
          <ul className="space-y-2.5 pl-2">
            <li className="flex items-start gap-2.5">
              ...
              <div>
                <strong className="text-slate-900 dark:text-zinc-100">Distributed & Background Workloads:</strong> Designing worker pools, idempotent message queues, and lease recovery engines (like <Link to="/projects/pulsemesh" className="text-slate-900 dark:text-zinc-100 font-semibold underline">PulseMesh</Link>).
              </div>
            </li>
            <li className="flex items-start gap-2.5">
              <span className="text-slate-400 dark:text-zinc-500 font-mono font-bold">→</span>
              <div>
                <strong className="text-slate-900 dark:text-zinc-100">Data-Intensive Applications:</strong> Timeseries analytics, sensor aggregation, and spatial indexing (like <Link to="/projects/varshanet" className="text-slate-900 dark:text-zinc-100 font-semibold underline">VARSHANET</Link> and <Link to="/projects/ocean-intelligence" className="text-slate-900 dark:text-zinc-100 font-semibold underline">Ocean Intelligence Platform</Link>).
              </div>
            </li>
            <li className="flex items-start gap-2.5">
              <span className="text-slate-400 dark:text-zinc-500 font-mono font-bold">→</span>
              <div>
                <strong className="text-slate-900 dark:text-zinc-100">Offline-First & Resilient Clients:</strong> Systems utilizing CRDTs and local indexed storage to stay 100% operational during connectivity loss (like <Link to="/projects/polaris" className="text-slate-900 dark:text-zinc-100 font-semibold underline">POLARIS</Link>).
              </div>
            </li>
            <li className="flex items-start gap-2.5">
              <span className="text-slate-400 dark:text-zinc-500 font-mono font-bold">→</span>
              <div>
                <strong className="text-slate-900 dark:text-zinc-100">Developer Tooling & Notes:</strong> Clean CLIs, reproducible environments, and detailed technical writing.
              </div>
            </li>
          </ul>
        </div>
      ...
      <section className="space-y-4">
        ...
        <div className="glass-card p-6 rounded-3xl space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs sm:text-sm">
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-zinc-800/40 border border-slate-200/80 dark:border-white/5">
              <span className="font-bold text-slate-900 dark:text-zinc-100 block mb-1">Distributed Consensus</span>
              <p className="text-slate-500 dark:text-zinc-400 text-xs">Studying Raft, vector clocks, and crash-recovery protocols.</p>
            </div>
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-zinc-800/40 border border-slate-200/80 dark:border-white/5">
              <span className="font-bold text-slate-900 dark:text-zinc-100 block mb-1">Modern C++ Memory Internals</span>
              <p className="text-slate-500 dark:text-zinc-400 text-xs">Move semantics, custom allocators, and cache locality profiling.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-4 pt-4 border-t border-slate-200/80 dark:border-zinc-800/80">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {profile.philosophy.map((rule, idx) => (
            <div
              key={idx}
              className="glass-card p-5 rounded-2xl text-xs sm:text-sm text-slate-700 dark:text-zinc-300 font-medium"
            >
              <span className="font-mono text-slate-400 mr-2 font-bold">0{idx + 1}.</span>
              {rule}
            </div>
          ))}
        </div>
      ...
      {/* Contact CTA */}
      <div className="pt-6 flex items-center justify-between border-t border-slate-200/80 dark:border-zinc-800/80">
        <span className="text-xs text-slate-500 font-mono">Want to work together or chat?</span>
        <Link
          to="/contact"
