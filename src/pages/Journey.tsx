  ...
  return (
    <div className="max-w-3xl mx-auto space-y-10 pb-16 pt-6">
      ...
      <div className="relative border-l-2 border-zinc-200 dark:border-zinc-800 ml-4 sm:ml-6 space-y-12">
        {journeyTimeline.map((item: JourneyItem, idx: number) => (
          <div key={item.year} className="relative pl-6 sm:pl-8 group">
            {/* Timeline Node Icon */}
            <div className="absolute -left-[9px] top-1.5 w-4 h-4 rounded-full border-2 border-white dark:border-[#0A0A0A] bg-emerald-500 group-hover:scale-125 transition-transform" />

            {/* Year & Quarter Badge */}
            <div className="flex items-center gap-2 mb-2">
              <span className="text-sm font-mono font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-2.5 py-0.5 rounded">
                {item.year}
              </span>
              {item.quarter && (
            ...
            <div className="p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#111111] space-y-4 shadow-sm">
              ...
              <div className="flex flex-wrap items-center gap-1.5 pt-2 border-t border-zinc-100 dark:border-zinc-800/80">
                {item.technologies.map((tech) => (
                  <span
                    key={tech}
                    className="px-2 py-0.5 text-[11px] font-mono rounded bg-zinc-100 dark:bg-zinc-800/80 text-zinc-600 dark:text-zinc-400"
                  >
                    {tech}
                  </span>
                ))}
