...
export const TerminalSignature: React.FC = () => {
  ...
  const [history, setHistory] = useState<HistoryItem[]>([
    {
      ...
      output: (
        <div className="text-zinc-300 dark:text-zinc-200">
          <span className="font-semibold text-emerald-400">{profile.name}</span> — {profile.role}
        </div>
      ),
    },
    {
    ...
    {
      command: 'status',
      output: <span className="text-amber-400 font-mono">learning & building... (continuous)</span>,
    },
  ]);
      ...
      case 'projects':
        output = (
          <div className="space-y-1 text-xs text-zinc-300">
            <p className="text-emerald-400 font-semibold">• PulseMesh: Distributed worker orchestrator</p>
            <p className="text-emerald-400 font-semibold">• VARSHANET: High-precision meteorological analytics</p>
            <p className="text-emerald-400 font-semibold">• POLARIS: Offline-first polar expedition logistics</p>
            <p className="text-emerald-400 font-semibold">• Ocean Intelligence: Marine anomaly monitor</p>
          </div>
        );
        break;
      case 'skills':
      ...
      case 'contact':
        output = (
          <div className="text-xs space-y-0.5 text-zinc-300">
            <p>Email: <a href={`mailto:${socialLinks.email}`} className="text-emerald-400 underline">{socialLinks.email}</a></p>
            <p>GitHub: <a href={socialLinks.github} target="_blank" rel="noreferrer" className="text-emerald-400 underline">{socialLinks.github}</a></p>
          </div>
        );
        break;
      case 'help':
        output = (
          <div className="text-xs text-zinc-400">
            Available commands: <span className="text-emerald-400 font-mono">{availableCommands.join(', ')}</span>
          </div>
        );
        break;
      default:
  ...
  return (
    <div className="w-full rounded-xl border border-zinc-200 dark:border-zinc-800 bg-[#0F0F11] text-zinc-100 shadow-xl overflow-hidden font-mono text-xs sm:text-sm">
      {/* Terminal Titlebar */}
      ...
      >
        ...
        <div className="flex items-center gap-2 text-zinc-400 pt-1">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-zinc-100 placeholder-zinc-600 focus:outline-none text-xs sm:text-sm font-mono caret-emerald-400"
          />
      ...
      <div className="px-4 py-2 bg-[#141416] border-t border-zinc-800/80 flex flex-wrap items-center gap-1.5 text-[11px] text-zinc-400">
        <span className="text-zinc-500 mr-1">Quick Run:</span>
        {availableCommands.map((cmd) => (
          <button
            key={cmd}
            onClick={() => executeCommand(cmd)}
            className="px-2 py-0.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-zinc-100 rounded text-[11px] transition-colors"
          >
            ${cmd}
          </button>
        ))}
